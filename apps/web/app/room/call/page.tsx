...
function CallInner() {
  ...
  const handleBufferFull = useCallback(async (frames: LandmarkFrame[]) => {
    try {
      ...
      if (data.status === 'ok' && data.predicted_gloss) {
        ...
        if (sameSign && tooSoon) {
          const remaining = Math.ceil((COOLDOWN_MS - (now - lastGlossTimeRef.current)) / 1000)
        ...
        setLastStatus(`Recognised: ${gloss} (${Math.round(data.confidence * 100)}%)`)
        await broadcastRef.current(gloss, data.confidence)

      } else if (data.status === 'low_confidence') {
        setLastStatus(`Signing detected — confidence too low (${Math.round(data.confidence * 100)}%)`)
          ...
          <div style={{
            ...
            <div style={{
              ...
              <div style={{
                ...
                height:       '8px',
                borderRadius: '50%',
                background:   handsDetected      ? '#4ade80'
                             : status === 'initialising' ? '#facc15'
                             : '#666',
              }} />
              <span style={{ fontWeight: 500 }}>
                {status === 'initialising' ? 'Loading AI...'
      ...
      <div style={{ color: '#fff', padding: '2rem', textAlign: 'center' }}>
        ...
        <button
          ...
          style={{
            ...
            borderRadius: '8px', background: '#4ade80',
